...
export type MeetingContactFieldsProps = {
  ...
  /** Virksomhed til mødet (Podio/Gecko) — separat fra leadets companyName. */
  meetingCompanyName?: string;
  onMeetingCompanyName?: (v: string) => void;
  meetingCompanyNameError?: string;
  className?: string;
...
export function MeetingContactFields({
  ...
  return (
    ...
    >
      ...
      {onMeetingCompanyName ? (
        <div>
          <label className="mb-1 block text-xs font-medium text-emerald-950">Virksomhed</label>
          <input
            type="text"
            autoComplete="organization"
            value={meetingCompanyName ?? ""}
            onChange={(ev) => onMeetingCompanyName(ev.target.value)}
            aria-invalid={Boolean(meetingCompanyNameError)}
            className={`w-full rounded-lg border bg-white px-3 py-2 text-sm text-stone-900 shadow-sm outline-none ${inputRingClass(meetingCompanyNameError)}`}
            placeholder="Fx Tandlægerne Hansen"
            required={contactRequired}
          />
          {meetingCompanyNameError ? (
            <p className="mt-1 text-xs text-red-600">{meetingCompanyNameError}</p>
          ) : null}
        </div>
      ) : null}
      <div>
