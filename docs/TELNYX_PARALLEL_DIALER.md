...
Se også [PREDICTIVE-VS-POWER-DEBUG.md](PREDICTIVE-VS-POWER-DEBUG.md) for Telnyx/Neon-verifikation (MCP), SQL-diagnostik og fejlspor til predictive vs. power.
...
- **Pacing i Allio:** `PREDICTIVE` på **kampagne-arbejd** bruger ikke længere heartbeat-dispatch; **Power**-kampagner bruger stadig dispatch med fast ratio (se tabellen under — i dag `POWER_DIALER_LEADS_PER_READY_AGENT` i koden, typisk 5). (Historisk predictive-dispatch med dynamisk ratio findes stadig i `lib/dialer-pacing.ts` hvis endpoint kaldes manuelt.)
...
| `POWER_DIALER_REQUEUE_COOLDOWN_MINUTES` *(valgfri)* | **Power Dialer:** efter usikkert AMD (`not_sure` / `silence` / …) eller no-bridge hangup uden AMD-udfald forbliver leadet `NEW`, men får `powerDialerEligibleAfter = now + N minutter` (default **10** hvis variablen mangler eller er ugyldig), så det ikke straks dispatches igen. |

### Power Dialer: venteskærm i arbejdsvindue (fejlsøgning)

Venteskærmen (kun progress bar m.m.) vises kun når **alle** disse er opfyldt:

1. Kampagnen er `POWER_DIALER`.
2. URL indeholder **`voipSession=1`** (eller `true`) — fx fra knappen **Start** på `/kampagner`, **ikke** et direkte link til `/kampagner/:id/arbejd` uden query.
3. Der er **ikke** et foretrukket lead: hverken `leadId` i URL **eller** `sessionStorage`-nøgle `kampagne-arbejd-prefer:<campaignId>` (et gammelt prefer kan overtage pladsen — ryd storage eller åbn med `voipSession=1` uden `leadId`).

**Produktion:** Bekræft i Vercel at det aktuelle deployment matcher den forventede `main`-build (ingen gammel cache — prøv hård genindlæsning). Uden ny frontend ser du et ældre layout.

### Power vs. Predictive: AMD «unknown»

| Mode | Usikkert AMD (`silence`, `not_sure`, …) |
|------|----------------------------------------|
| `PREDICTIVE` | Bridges stadig til agent (som før). |
| `POWER_DIALER` | Ingen bridge — opkald afsluttes, lead **requeue** bagerst med cooldown (`Lead.powerDialerEligibleAfter`). |
| `POWER_DIALER` maskine/fax | `VOICEMAIL` (som før via `handleAmdMachine`). |
...
|------|--------------|------------|
| `POWER_DIALER` | Fast (i koden: `POWER_DIALER_LEADS_PER_READY_AGENT`, typisk 5) | Fast ratio pr. «ready» agent — parallel lead-ben op til cap; justeres i `lib/dialer-dispatch-math.ts`. |
| `PREDICTIVE` | 1.0–3.0 (dynamisk) | **Kun** når `POST /api/dialer/dispatch` kaldes (fx manuelt/admin). I **campaign-workspace** enabler predictive **ikke** dispatch; ét WebRTC-opkald ad gangen. |
...
Dispatcheren kaldes af klienten ved hver heartbeat (5 sek) for **`POWER_DIALER`**-kampagner når agenten er `ready` og auto-dial ikke er pauset. `PREDICTIVE` på arbejdssiden kalder ikke dispatch fra workspace. Dispatcher-kaldet er idempotent og placerer kun nye opkald hvis pacing-budgettet tillader det.
...
| Scenario | Trigger |
|----------|---------|
| Power Dialer / Predictive (server-dispatched) | Når AMD-resultatet er **menneske** (premium: `human_residence` / `human_business` / `human`). **Predictive** bridger også ved usikkert AMD (`unknown`). **Power Dialer** bridger **ikke** ved `unknown` (requeue i stedet). Optagelse starter på lead-leggen efter bridge til agent. |
